...
| `record_lookup.order_column` | `'updated_at'` | Column the record-select orders its initial page and search results by. Must be a real column on the looked-up model. |
...
When `order_column` is left at `updated_at` and the looked-up model returns `false` from
`usesTimestamps()`, the model key is used instead, so the order is always deterministic.
A column you configure explicitly is used as given.
